...
WORK_STATE ossl_statem_server_pre_work(SSL *s, WORK_STATE wst)
{
    ...
    case TLS_ST_SW_SESSION_TICKET:
        ...
        if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->sent_tickets == 0) {
            /*
...
WORK_STATE ossl_statem_server_post_work(SSL *s, WORK_STATE wst)
{
    ...
    case TLS_ST_SW_SRVR_HELLO:
        if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->hello_retry_request == SSL_HRR_PENDING) {
            if ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) == 0
                    && statem_flush(s) != 1)
                return WORK_MORE_A;
            break;
    ...
    case TLS_ST_SW_CHANGE:
        ...
        if (SSL_IS_DTLS(s) && !SSL_IS_DTLS13(s))
            dtls1_reset_seq_numbers(s, SSL3_CC_WRITE, -1);
        break;
    ...
    case TLS_ST_SW_SESSION_TICKET:
        clear_sys_error();
        if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && statem_flush(s) != 1) {
...
static int tls_early_post_process_client_hello(SSL *s)
{
     ...
     */
    if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && s->hit) {
        j = 0;
    ...
    if (!s->hit || SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) {
...
WORK_STATE tls_post_process_client_hello(SSL *s, WORK_STATE wst)
{
    ...
    if (wst == WORK_MORE_B) {
         ...
         */
        if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && !tls_handle_alpn(s)) {
            /* SSLfatal() already called */
            goto err;
        }
...
int tls_construct_server_hello(SSL *s, WPACKET *pkt)
{
    ...
    int usetls13 = SSL_IS_TLS13(s) || SSL_IS_DTLS13(s) || s->hello_retry_request == SSL_HRR_PENDING;

    if (SSL_IS_DTLS(s))
        version = usetls13 ? DTLS1_2_VERSION : s->version;
    else
    ...
    if (!tls_construct_extensions(s, pkt,
                                  s->hello_retry_request == SSL_HRR_PENDING
                                      ...
                                      : ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s))
...
int tls_construct_server_key_exchange(SSL *s, WPACKET *pkt)
{
...
#ifndef OPENSSL_NO_CNSM
    if (type & SSL_kSM2) {
        enc_cert = BUF_MEM_new();
        if (!enc_cert) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE,
                        ERR_R_MALLOC_FAILURE);
            goto err;
        }
        enc_cert_len = i2d_X509((&s->cert->pkeys[SSL_PKEY_SM2_ENC])->x509, NULL);
        if (enc_cert_len < 0 || !BUF_MEM_grow_clean(enc_cert, (int)(enc_cert_len + 3))) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE,
                        ERR_R_INTERNAL_ERROR);
            goto err;
        }
        p = (unsigned char *)&(enc_cert->data[0]);
        l2n3(enc_cert_len, p);
        enc_cert_len = i2d_X509((&s->cert->pkeys[SSL_PKEY_SM2_ENC])->x509, &p);
        if (enc_cert_len < 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE,
                        ERR_R_INTERNAL_ERROR);
            goto err;
        ...
        s->s3->tmp.pkey = ssl_generate_pkey_group(s, 0);
        /* Generate a new key for this curve */
        if (s->s3->tmp.pkey == NULL) {
            /* SSLfatal() already called */
            goto err;
    ...
    if (type & (SSL_kECDHE | SSL_kECDHEPSK | SSL_kSM2DH)) {
        ...
        if (!WPACKET_put_bytes_u8(pkt, NAMED_CURVE_TYPE)
                ...
                || !WPACKET_put_bytes_u8(pkt, (type & SSL_kSM2DH) ? SM1_1_VERSION_DEFAULT_SM2_CURVE_ID : curve_id)
    ...
    if (lu != NULL) {
        ...
        tbslen = construct_key_exchange_tbs(s, &tbs,
                                            (type & SSL_kSM2) ? (const void *)(enc_cert->data) : s->init_buf->data + paramoffset,
                                            (type & SSL_kSM2) ? enc_cert_len : paramlen);
        if (tbslen == 0) {
            /* SSLfatal() already called */
            goto err;
        }
...
static int tls_process_cke_rsa(SSL *s, PACKET *pkt)
{
    ...
    if(EVP_PKEY_pmeth_engine(pkey)){
        pkey_ctx = EVP_PKEY_CTX_new_pkey_id(pkey, NID_rsaEncryption, NULL);
    	if (pkey_ctx == NULL || EVP_PKEY_decrypt_init(pkey_ctx) <= 0) {
    	    SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_CKE_RSA,
    	             ERR_R_INTERNAL_ERROR);
    	    goto err;
    	}
    	decrypt_len_size_t = RSA_size(rsa);
        if (EVP_PKEY_decrypt(pkey_ctx, rsa_decrypt, (size_t *)&decrypt_len_size_t, PACKET_data(&enc_premaster), (int)PACKET_remaining(&enc_premaster)) <= 0)
        {
            SSLfatal(s, SSL_AD_DECRYPT_ERROR, SSL_F_TLS_PROCESS_CKE_RSA,
                        ERR_R_INTERNAL_ERROR);
            goto err;
        }
        decrypt_len = decrypt_len_size_t;
        goto FIN;
    }
#endif
...
MSG_PROCESS_RETURN tls_process_client_certificate(SSL *s, PACKET *pkt)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && (!PACKET_get_length_prefixed_1(pkt, &context)
    ...
    if (s->version == SM1_1_VERSION && x_enc) {
     ...
     * message
     */
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && !ssl3_digest_cached_records(s, 1)) {
        /* SSLfatal() already called */
        goto err;
    }
...
int tls_construct_server_certificate(SSL *s, WPACKET *pkt)
{
     ...
     */
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && !WPACKET_put_bytes_u8(pkt, 0)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                 SSL_F_TLS_CONSTRUCT_SERVER_CERTIFICATE, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    if (s->version == SM1_1_VERSION)
    {
        /* double cert for GMTLSv1.1 */
        if (!ssl3_output_sm2_cert_chain(s, pkt,  s->cert->key, &s->cert->pkeys[SSL_PKEY_SM2_ENC])) {
            return 0;
        }
    }
    else
