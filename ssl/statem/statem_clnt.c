...
static int key_exchange_expected(SSL *s)
{
     ...
     */
	    if (alg_k & (SSL_kDHE | SSL_kECDHE | SSL_kDHEPSK | SSL_kECDHEPSK | SSL_kSM2 | SSL_kSM2DH
                 | SSL_kSRP)) {
        return 1;
    }
...
static int set_client_ciphersuite(SSL *s, const unsigned char *cipherchars)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->s3->tmp.new_cipher != NULL
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL *s, PACKET *pkt)
{
    ...
    if (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s) || hrr) {
        if (compression != 0) {
            SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER,
     ...
     */
    context = (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) ? SSL_EXT_TLS1_3_SERVER_HELLO
                              : SSL_EXT_TLS1_2_SERVER_HELLO;
...
MSG_PROCESS_RETURN tls_process_server_certificate(SSL *s, PACKET *pkt)
{
    ...
    if (((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && !PACKET_get_1(pkt, &context))
            || context != 0
...
static int tls_process_ske_ecdhe(SSL *s, PACKET *pkt, EVP_PKEY **pkey)
{
     ...
     */
    if (s->s3->tmp.new_cipher->algorithm_mkey & SSL_kSM2DH) {
        if ((s->s3->peer_tmp = ssl_generate_param_group_sm2()) == NULL) {
     ...
     * and ECDSA.
     */
    if (s->s3->tmp.new_cipher->algorithm_auth & (SSL_aECDSA | SSL_aSM2DSA))
        *pkey = X509_get0_pubkey(s->session->peer);
...
MSG_PROCESS_RETURN tls_process_key_exchange(SSL *s, PACKET *pkt)
{
    ...
    if (alg_k & (SSL_kPSK | SSL_kRSAPSK)) {
        ...
        enc_cert_len = i2d_X509(s->session->peer_enc, NULL);
        ...
        l2n3(enc_cert_len, p);
        enc_cert_len = i2d_X509(s->session->peer_enc, &p);
        if (enc_cert_len < 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_KEY_EXCHANGE,
                        ERR_R_INTERNAL_ERROR);
            goto err;
    ...
    if (pkey != NULL) {
        ...
        tbslen = construct_key_exchange_tbs(s, &tbs, (alg_k & SSL_kSM2) ? (const void *)(enc_cert->data) : PACKET_data(&params),
                                            (alg_k & SSL_kSM2) ? enc_cert_len : PACKET_remaining(&params));
        if (tbslen == 0) {
            /* SSLfatal() already called */
            goto err;
        }
...
MSG_PROCESS_RETURN tls_process_certificate_request(SSL *s, PACKET *pkt)
{
     ...
     * client_cert_cb.
     */
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->post_handshake_auth != SSL_PHA_REQUESTED)
        return MSG_PROCESS_CONTINUE_READING;
...
static int tls_construct_cke_sm2(SSL *s, WPACKET *pkt)
{
    unsigned char *encdata = NULL;
    EVP_PKEY *pkey = NULL;
    EVP_PKEY_CTX *pctx = NULL;
    ...
    if (s->session->peer_enc == NULL) {
        /*
         * We should always have a server encrypt certificate.
         */
    ...
    /* Set length after encrypt, since length get from 
     * EVP_PKEY_encrypt(pctx, NULL, &enclen, pms, pmslen)
     * not exactly equal to the real length, maybe great 
     * or equal to real length, in SM2 algorithm */
    pkt->written += enclen;
    pkt->curr += enclen;
...
static int tls_construct_cke_sm2dh(SSL *s, WPACKET *pkt)
{
    ...
    if (EVP_PKEY_CTX_ctrl(pctx, -1, EVP_PKEY_OP_PARAMGEN|EVP_PKEY_OP_KEYGEN,
    ...
    if (!WPACKET_put_bytes_u8(pkt, NAMED_CURVE_TYPE)
                ...
                || !WPACKET_put_bytes_u8(pkt, SM1_1_VERSION_DEFAULT_SM2_CURVE_ID)
...
int tls_construct_client_key_exchange(SSL *s, WPACKET *pkt)
{
    ...
    if (alg_k & (SSL_kRSA | SSL_kRSAPSK)) {
        ...
        if (!tls_construct_cke_sm2(s, pkt))
          	goto err;
...
int ssl3_check_cert_and_algorithm(SSL *s)
{
    ...
    if (clu->amask & SSL_aSM2DSA) {
        if (ssl_check_srvr_sm2_cert_and_alg(s->session->peer, s))
            return 1;
        SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
                 SSL_F_SSL3_CHECK_CERT_AND_ALGORITHM, SSL_R_BAD_SM2_CERT);
        return 0;
    }
