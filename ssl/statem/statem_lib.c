...
int tls_construct_cert_verify(SSL *s, WPACKET *pkt)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && EVP_PKEY_id(pkey) == EVP_PKEY_SM2) {
        if (pkey != NULL) {
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL *s, PACKET *pkt)
{
     ...
     */
    if (!s->server && (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->s3->tmp.cert_req == 1)
...
int tls_construct_finished(SSL *s, WPACKET *pkt)
{
     ...
     */
    if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && !ssl_log_secret(s, MASTER_SECRET_LABEL,
                                            s->session->master_key,
...
int tls_construct_ack(SSL *s, WPACKET *pkt)
{
    unsigned char ack[SEQ_NUM_SIZE], *p = ack;
    size_t acklen = SEQ_NUM_SIZE;
    memcpy(p, s->rlayer.read_sequence, SEQ_NUM_SIZE);
    s2n(s->rlayer.d->r_epoch-1, p);
    if (!WPACKET_sub_memcpy_u16(pkt, ack, acklen)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_ACK,
                 ERR_R_INTERNAL_ERROR);
        return 0;
    }
    return 1;
...
MSG_PROCESS_RETURN tls_process_ack(SSL *s, PACKET *pkt)
{
    return MSG_PROCESS_FINISHED_READING;
}
...
MSG_PROCESS_RETURN tls_process_change_cipher_spec(SSL *s, PACKET *pkt)
{
    ...
    if (SSL_IS_DTLS(s)) {
        if (!SSL_IS_DTLS13(s))
          dtls1_reset_seq_numbers(s, SSL3_CC_READ, -1);
...
MSG_PROCESS_RETURN tls_process_finished(SSL *s, PACKET *pkt)
{
    ...
    if (s->server) {
        ...
        */
        ...
        if (s->post_handshake_auth != SSL_PHA_REQUESTED)
            s->statem.cleanuphand = 1;
        if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s))  && !tls13_save_handshake_digest_for_pha(s)) {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
        }
    }
     ...
     */
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && RECORD_LAYER_processed_read_pending(&s->rlayer)) {
    ...
    if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && !s->s3->change_cipher_spec) {
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_TLS_PROCESS_FINISHED,
...
WORK_STATE tls_finish_handshake(SSL *s, WORK_STATE wst, int clearbufs, int stop)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && !s->server
...
static int ssl_method_error(const SSL *s, const SSL_METHOD *method)
{
    ...
    if (0 == version_cmp(s, s->client_version, SM1_1_VERSION))
      return 0;
...
int ssl_choose_server_version(SSL *s, CLIENTHELLO_MSG *hello, DOWNGRADE *dgrd)
{
    ...
    if (suppversions->present) {
        ...
        unsigned int best_vers = (SSL_IS_DTLS(s) ? 0xFFFF : 0);
         ...
         */
        if (!SSL_IS_DTLS(s) && client_version <= SSL3_VERSION)
...
int ssl_choose_client_version(SSL *s, int version, RAW_EXTENSION *extensions)
{
    ...
    if ((!SSL_IS_DTLS(s) && s->version == TLS1_2_VERSION && real_max > s->version)
                || (SSL_IS_DTLS(s) && s->version == DTLS1_2_VERSION && DTLS_VERSION_GT(real_max, s->version))) {
        if (memcmp(tls12downgrade,
            ...
            return 0;
                ...
                || (SSL_IS_DTLS(s) 
                    ...
                    && DTLS_VERSION_GT(real_max, s->version))) {
...
int ssl_set_client_hello_version(SSL *s)
{
    ...
    else if (SSL_IS_DTLS(s) && DTLS_VERSION_GT(ver_max, DTLS1_2_VERSION))
        ver_max = DTLS1_2_VERSION;
