...
static int tls1_check_pkey_comp(SSL *s, EVP_PKEY *pkey)
{
    ...
    } else if (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) {
            /*
...
static const uint16_t tls12_sigalgs[] = {
    ...
    TLSEXT_SIGALG_sm2sig_sm3, /* rfc 8998 */
...
static const SIGALG_LOOKUP sigalg_lookup_tbl[] = {
     ...
     NID_sm3WithSM2Sign, NID_sm2}, /* rfc 8998 */
...
static const uint16_t tls_default_sigalg[] = {
    ...
    0, /* SSL_PKEY_ED25519 */
    0, /* SSL_PKEY_ED448 */
    TLSEXT_SIGALG_sm2sig_sm3, /* SSL_PKEY_SM2 */
    TLSEXT_SIGALG_sm2sig_sm3, /* SSL_PKEY_SM2_ENC */
};
...
int tls12_check_peer_sigalg(SSL *s, uint16_t sig, EVP_PKEY *pkey)
{
     ...
     */
    if (lu == NULL
        || ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && (lu->hash == NID_sha1 || lu->hash == NID_sha224))
        || (pkeyid != lu->sig
    ...
    if (pkeyid == EVP_PKEY_EC) {
        ...
        if (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s) || tls1_suiteb(s)) {
            EC_KEY *ec = EVP_PKEY_get0_EC_KEY(pkey);
                                             ...
                                             SSL_SESSION **ret)
{
    ...
    *ret = NULL;
     ...
     * resumption.
     */
    if ((s->version <= SSL3_VERSION && s->version != SM1_1_VERSION) || !tls_use_ticket(s))
                                     ...
                                     size_t sesslen, SSL_SESSION **psess)
{
    ...
    if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && s->ext.session_secret_cb) {
        /*
    ...
    if (s->ext.session_secret_cb == NULL || SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) {
        switch (ret) {
...
static int tls12_sigalg_allowed(const SSL *s, int op, const SIGALG_LOOKUP *lu)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && lu->sig == EVP_PKEY_DSA)
        return 0;
    ...
    if (lu->sig == NID_id_GostR3410_2012_256
        ...
        if (s->server && (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)))
            return 0;
        if (!s->server
            ...
            if (!SSL_IS_DTLS(s) && s->s3->tmp.min_ver >= TLS1_3_VERSION)
                return 0;
...
int tls1_process_sigalgs(SSL *s)
{
    ...
    for (i = 0; i < s->shared_sigalgslen; i++) {
        ...
        if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && sigptr->sig == EVP_PKEY_RSA)
            continue;
...
static int tls1_check_sig_alg(SSL *s, X509 *x, int default_nid)
{
    ...
    if ((SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) && s->s3->tmp.peer_cert_sigalgs != NULL) {
        /*
